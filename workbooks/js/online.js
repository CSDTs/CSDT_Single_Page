...
function initCloudServices() {
	...
	fetchCurrentUser()
		.then((data) => {
			...
			if (currentUser.loggedIn) {
				fetchUserClassroomList(
					...
					(err) => {
						console.error(`Error: Failed to get a classroom list -- ${JSON.stringify(err)}`);
					}
				);
			}
			// Update layout and project list status

			$(userInteractions.validateField).attr("placeholder", currentUser.name);
			// TODO: Validate if the user can access the file with or without being logged in
			checkForConfigFile();
			updateGlobalLayout();
			updateProjectList();
		})
		.catch((err) => {
			...
			console.error(`Error: Failed to get a current user -- ${JSON.stringify(err)}`);
		});
}
...
function createLoadProjectComponent(state) {
	...
	if (state == "checking") {
		...
		loader.setAttribute("src", `${isAppHomepage ? currentLocation : ""}img/loader.svg`);
	...
	if (state == "loading") {
		...
		select.addEventListener("change", (e) => {
			let optionAttributes = {
				classroom: e.target.options[e.target.selectedIndex].getAttribute("data-classroom"),
				classroom_name: e.target.options[e.target.selectedIndex].getAttribute("data-classroom_name"),
				screenshot_url: e.target.options[e.target.selectedIndex].getAttribute("data-screenshot_url"),
				when_modified: e.target.options[e.target.selectedIndex].getAttribute("data-when_modified"),
				when_created: e.target.options[e.target.selectedIndex].getAttribute("data-when_created"),
			};
...
function successfullyFetchedProjects(projects) {
	...
	projects.forEach((project) => {
		$(`#${cloudUI.loadProjectList}`).append(createNewOption(project, "project"));
	});
	...
	document.querySelector(`#${cloudUI.loadProjectList} [value="${mostRecentProject.id}"]`).selected = true;
...
function errorFetchingProjects(err) {
	console.error(`Error Message: Failed to fetch current user's projects -- ${JSON.stringify(err)}`);
...
function handleLoadProjectOptions(state) {
	...
	if (state == "empty" || state == "error" || state == "checking") loginBtnStatus = true;
...
function updateProjectDetails(data, state) {
	...
	loadProjectClassroom.innerHTML =
		data.classroom == "undefined" || data.classroom == null || data.classroom == "null"
...
function loadProjectFromCloud(id, serializeCallback) {
	...
	let successfulProjectLoad = function (data, proj) {
		...
		let awsData = typeof data === "string" ? proj : JSON.stringify(proj);
		serializeCallback(awsData);
		...
		$(`#${cloudUI.visibleModifiedStatus}`).html(`Last saved on: <strong>${currentProject.created_at}</strong>`);
...
function fetchUserProjectList(success, error) {
	$.get(`${csdtAPI.project}?owner=${currentUser.id}&application=${applicationID}`, null, success, "json").fail(error);
}
...
function saveProjectToCloud() {
	...
	let projectForm = generateSaveFormData(saveObject.project, "application/json");
...
function updateNavigationBar() {
	...
	$(`#${cloudUI.navUserProjects}`).attr("href", currentUser.loggedIn ? currentUserProfileURL : "");
	$(`#${cloudUI.navUserClassrooms}`).attr("href", currentUser.loggedIn ? currentUserClassroomsURL : "");
	$(`#${cloudUI.navSignOut}`).attr("hidden", !currentUser.loggedIn);

	$(`#${cloudUI.navUserStatus}`).addClass(currentUser.loggedIn ? "dropdown-toggle" : "");
	$(`#${cloudUI.navUserStatus}`).removeClass(!currentUser.loggedIn ? "dropdown-toggle" : "");
	$(`#${cloudUI.navUserStatus}`).attr("data-toggle", currentUser.loggedIn ? "dropdown" : "modal");
	$(`#${cloudUI.navUserStatus}`).attr("data-target", currentUser.loggedIn ? "" : "#signInPrompt");
	$(`#${cloudUI.navUserStatus}`).attr("aria-expanded", false);
	// $(`#${cloudUI.navUserStatus}`).attr('aria-haspopup', currentUser.loggedIn);
	$(`#${cloudUI.navUserDropdown}`).removeClass(!currentUser.loggedIn ? "show" : "");
	$(`#${cloudUI.navUserContainer}`).removeClass(!currentUser.loggedIn ? "show" : "");
	if (!currentUser.loggedIn) {
...
function updateSaveProjectPrompt() {
	...
	$(`#${cloudUI.saveConfirmMsg}`).html(
		currentUser.loggedIn ? "Are you sure you want to replace your current project?" : "Sign in to save your work."
	);
}
...
function attemptSignOut() {
	...
	submitSignOutRequest()
		...
		.catch((err) => {
			...
			alertUser("There was an error when signing you out. Please try again.", 3500);
			//Log the issue
...
function emptyClassroomList() {
	$(`#${cloudUI.saveProjectClassroomSelect}`).empty().append("<option>Choose...</option>");
}
function updateSignOutPrompt(status) {
	...
	// Show the form
	$(`#${cloudUI.signOutPrompt} div div div.modal-body`).attr("hidden", status == "show");
...
function updateSignInPrompt(status) {
	...
	// Hide the form
	$(`#${cloudUI.signInPrompt} div div div.modal-body`).attr("hidden", status == "show");
