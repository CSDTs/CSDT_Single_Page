...
class WheelsContainer {
	constructor(defaultWheels) {
		...
		this.wheelsParentContainer = document.getElementById(appReferences.wheelsContainer);

		this.numberOfWheelsSelect = document.getElementById(appReferences.numOfWheels);
	...
	setWheelConfiguration(opts) {
		...
		// Whichever is lower.
		this.setVisibleWheelCount(Math.min(Object.keys(opts).length, TOTAL_WHEEL_COUNT));
	...
	setVisibleWheelCount(num) {
		this.wheelCount = num;
		this.wheelControlPanels = document.querySelectorAll(`${appReferences.wheelControlsClass}:not(#recording-controls)`);
		...
		this.rapWheel.controlPanel.style.display = globals.outgoingAudio == "" ? "none" : "block";
	}
...
class Wheel {
	constructor(wheelID) {
		...
		this.createHeardSoFar();
	}

	createHeardSoFar() {
		this.tracker = document.createElement("p");
		this.tracker.style.color = "cyan";

		this.tracker.innerHTML = `<i class="fas fa-chevron-circle-up"></i> <br/> 0`;
		this.tracker.style.marginTop = `${190 + (this.nodeCount < 8 ? this.nodeCount * 6 : 70)}px`;
	...
	createWheelControls(title = `Wheel ${this.wheelID + 1}:`) {
		...
		document.getElementById(appReferences.wheelControlsContainer).appendChild(this.controlPanel);
	}
	...
	createRepeatField() {
		...
		this.repeatLabel.innerHTML = "Loop: &nbsp;";
		...
		// this.repeatContainer.style.marginTop = `${190 + (this.nodeCount < 8 ? this.nodeCount * 6 : 70)}px`;
	...
	setNumOfBeats(nodeCount) {
		...
		if (this.tracker) {
			//Auto adjust based on number of tiles in wheel (quick calculation based on visual adjustments)
			this.tracker.style.marginTop = `${190 + (nodeCount < 8 ? nodeCount * 6 : 70)}px`;
		}
		// this.repeatContainer.style.marginTop = `${190 + (nodeCount < 8 ? nodeCount * 6 : 70)}px`;
	...
	update() {
		...
		if (this.isPlaying) {
			...
			this.currentBeatCountLabel.innerHTML = `Heard so far: <strong>${Math.ceil(this.currentNode)}</strong>`;
			this.tracker.innerHTML = `<i class="fas fa-chevron-circle-up"></i> <br/> ${Math.ceil(this.currentNode)}`;
		}
...
class RecordWheel {
	constructor() {
		...
		this.startRecordButton = document.getElementById(appReferences.recordButton);
		this.stopRecordButton = document.getElementById(appReferences.stopRecordButton);
		this.recordedAudioControls = document.getElementById(appReferences.recordedAudio);
		this.recordingCountdown = document.getElementById(appReferences.recordCountdown);

		this.parentContainer = document.getElementById(appReferences.wheelsContainer);

		this.controlsContainer = document.getElementById(appReferences.wheelControlsContainer);
	...
	testRecordingModal() {
		...
		this.recordingContainer = document.getElementsByClassName("recording-view")[0];
		...
		this.playbackButton = document.getElementsByClassName("recording-playback")[0];
	...
	addEventListeners() {
		...
		this.startRecordButton.addEventListener("click", () => this.promptRecordingCountdown());
	...
	startRecording() {
		...
		navigator.mediaDevices
			...
			.then((stream) => {
				...
				this.audioRec.addEventListener("dataavailable", (e) => this.processMicrophoneAudio(e));
				this.audioRec.start();
	...
	processSavedAudio(userAudioBlob) {
		...
		this.recordedAudioControls.setAttribute("src", URL.createObjectURL(userAudioBlob));
		this.recordedAudioControls.pause();
	...
	fillRecordedAudioBuffer(toExportIn = false) {
		...
		if (!toExportIn) this.recordedBufferSource = superAudioContext.createBufferSource();
		...
		for (let j = 0; j < this.loopCount; ++j) {
			setRecordWheel.set(recordedRawBuffer, j * 48000 * globals.recordAudioDuration);
		}
